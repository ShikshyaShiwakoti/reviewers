...
import { z } from 'zod'
import { createPostDto, createPostDtoBody } from '../validators/create-Post.validators'
import { NextFunction } from 'express'

export const createRestaurants = async (body: z.infer<typeof createPostDtoBody>, userId: number) => {
    const {title} = body
...
export const getAll = async (userId: number) => {
        return await prisma.restaurantsById.findMany({
            where: {userId: userId}
        })
}
...
export const updaterestturantsById = async (id: number, todo: any, loggedInUserId: number) => {
...
export const deleteById = async (req:Request, res:Response,next:NextFunction) => {
    try{
        ...
        if(todo.userId !== restaurantsById){
